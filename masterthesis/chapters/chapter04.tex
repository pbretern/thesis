...
7 Anwendungsfälle für das System beschrieben, die es zu erfüllen hat.
...
Die Bibliotheksleitung und die Bibliotheksmitarbeiter:innen können sich relevante \textit{\acrshort{KPI}} durch das System mit
...
Im Folgenden werden die funktionalen Anforderungen an das System formuliert. Das System strukturiert sich in drei Bereiche.
Die \autoref{tab:funktionale Anforderungen I} beschreibt die Anforderungen an das System bezüglich des \textit{\acrshort{ETL}-Bereiches} und der Datenspeicherung.
...
\begin{table}[H]
    ...
    \begin{tabular}{lp{14cm}c}
       ...
       \midrule
        ...
        F2                               &Das System verarbeitet und speichert die Daten automatisch.  & must\\
        F3                               &Das System verarbeitet die Daten mit Daten aus anderen Datenquellen.  & must\\
        F4                               &Das System speichert automatisch die Daten für ein Backup redundant.  & should\\
        %F40                             &Das System verschiebt die importierten Daten für das Backup.  & could\\
        F5                              &Das System löscht unnötige Daten am Ende des Import-Prozesses automatisch.  & could\\
    \bottomrule
    \end{tabular}
    \end{adjustbox}
    \caption{%
...
\begin{table}[H]
    ...
    \begin{tabular}{lp{14cm}c}
       ...
       \textbf{ID}          & \textbf{Beschreibung} &\textbf{Priorisierung}\\
       \midrule
        F6                               &Das System bietet \acrfull{GUI} an.  & must\\
        F7                               &Das System analysiert die Daten automatisch nach deskriptiven und explorativen Methoden der Statistik.  & must\\
        F8                               &Das System filtert die Daten nach bestimmten Kriterien.  & must\\ %Filterkriterien sind in Tabelle aufgeführt.
        F9                               &Das System bietet die Filterung der Daten über die \textit{\acrshort{GUI}} an.  & must\\
...
Die Anforderungen für die Datenpräsentation und die Erstellung des Standardberichtes beschreibt die \autoref{tab:funktionale Anforderungen III}.
...
\begin{table}[H]
    ...
    \begin{tabular}{lp{14cm}c}
       ...
       \textbf{ID}          & \textbf{Beschreibung} &\textbf{Priorisierung}\\
       \midrule
        F11                              &Das System bietet für die betreffenden Daten Datenvisualisierungen an. & must\\
        F12                              &Das System bietet für die betreffenden Daten eine Auswahl an Datenvisualisierungen an. & should\\
        F13                              &Das System erstellt bedarfsorientiert eine PDF mit den relevanten \textit{\acrshort{KPI}}. & must\\
        F14                              &Das System speichert das PDF-Dokument. & could\\
        F15                              &Das System speichert bedarfsorientiert Diagramme der relevanten \textit{\acrshort{KPI}} als Bilder. & could\\
        F16                              &Das System öffnet das PDF-Dokument automatisch. & must\\
        \bottomrule
    \end{tabular}
    \end{adjustbox}
    \caption{%
...
Evaluationstypen referiert. Für die Entwicklung des proof-concepts wurde eine Auswahl aus den bibliothekarischen Dienstleistungsbereichen getroffen.
