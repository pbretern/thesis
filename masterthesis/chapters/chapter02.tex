...
erfassten Informationen beruhen. Aus technischer Sicht umfasst \textit{\acrshort{BI}} die Komponenten Datenhaltung, Datenintegration, Reporting,
Data Mining etc. \textit{\acrshort{BI}-Systeme} gehören zu den Management-Support-Systemen, die seit den 1960er Jahren in Unternehmen zum Einsatz kommen.\cite[vgl.][83]{gronwald_integrierte_2020}

Der Begriff beschreibt nach \citeauthor{abts_grundkurs_2017} einen integrierten, unternehmensspezifischen,
IT-basierten Gesamtansatz zur Unterstützung betrieblicher Entscheidungen \cite[vgl.][270]{abts_grundkurs_2017}. 
Populär wurde er \textit{\acrshort{BI}} in den 1990er Jahren und fiel zusammen mit der 
...
Verdichtung von Unternehmensdaten auf unterschiedlichen Stufen mit detaillierten als auch aggregierten Kenzahlen.
...
Anwendungssystemen \cite[vgl.][267]{abts_grundkurs_2017}. 

\textit{\acrshort{BI}-Systeme} sind nach einer Referenzarchitektur/modell aufgebaut. Dieses Modell umfasst alle technischen Komponenten, die von der konkreten Implementierung aber abstrahiert werden.
Die Referenzarchitektur erfasst auch die Datenflüsse zwischen den verteilten Systemeinheiten und Komponenten\cite[vgl.][126 ff.]{linden_geschaftsmodellbasierte_2016}.
...
%Mit einem ETL-Prozess werden die heterogenen Daten aus den verschiedenen operativen Anwendungssystemen in einer Datenbank zusmmengeführt.

%\Citeauthor{linden_geschaftsmodellbasierte_2016} beschreibt die Architekttur mit drei Ebenen. 
%Diese dienen als eigentliche Datenlieferanten für die \textit{\acrshort{BI}-Systeme}.
 
%Es existiert ein Referenzmodell/ eine Referenzarchitektur / BI-Ordnnungsrahmen wie ein \acrshort{BI}-System aufgebaut ist.
%Dabei erstreckt sich der IT-basierte Gesamtansatz von der Datenbereitstellung/Speicherung über die Informationsgenerierung bis hin zum Informationzugriff
...
%Gleichwohl umfasst ein Referenzmodell auch die Datenflüsse zwischen den verteilten Systemeinheiten und Komponenten
%\Citeauthor{linden_geschaftsmodellbasierte_2016} beschreibt die Architekttur mit drei Ebenen. Die Ebene der Datenquellen- und Datenerfassung, die Ebene der Datenbereitstellung und die Ebene der Datenanalyse und Datenpräsentation.
...
%Der ETL-Prozess sorgt für die Herstellung einer inhaltlich konsistenten Datenbasis.
Die Ebenen umfassen die Ebene der Datenquellen und Datenerfassung, die Ebene der Datenbereitstellung und Speicherung sowie der Datenanalyse und -präsentation.

Auf der ersten Ebene werden die Daten mithilfe eines \acrfull{ETL}-Prozesses bearbeitet.
In einem ersten Schritt werden die Daten aus den operativen Vorsystemen extrahiert. Der anschließende Transformationsprozess wandelt die Daten in 
ein homogenes Format um. Dabei werden die Daten mit Hilfe  zum Teil automatischer Verfahren bereinigt, harmonisiert, 
aggregiert und angereichert \cite[vgl.][277 f.]{abts_grundkurs_2017}. Schließlich werden die Daten im Ladeprozess in ein \acrfull{DWH} geladen \cite[vgl.][129 ff.]{linden_geschaftsmodellbasierte_2016}. 

Das \textit{\acrshort{DWH}} entspricht der zweiten Ebene und ist nach \citeauthor{linden_geschaftsmodellbasierte_2016} der zentrale Bereich der BI-Referenzarchitekur. 
Ein Data Ware House ist ein logisch zentralisiertes, unternehmensweites Datenhaltungssystem, das physisch 
von den operativen Vorsystemen getrennt, eine harmonisierte Datenbasis darstellt. Es bietet eine dauerhafte 
...
In die Konzeptionierung eines \textit{\acrshort{DWH}s} sollten ein Archivierungskonzept für die Daten und Überlegungen zu den Aktualisierungszyklen der Datenbank 
miteinfließen. Die Aktualisierungszyklen legen fest, in welcher periodischen Abfolge die Daten aktualisiert werden, entweder zu Zeitpunkten der Änderungshäufigkeit
der Daten im Vorsystem, in periodischen Zeitabstände oder aber auch in Echtzeit.
...
Die dritte Ebene beschreibt die Ebene der Datenanalyse - und präsentation. Es gibt verschiedene Auswertungskonzepte, die in der Datenanalyse zur
Anwendung kommen. Im Rahmen der \textit{\acrshort{BI}-Systeme} wird häufig \acrfull{OLAP} angewendet. OLAP ist eine Anfragetechnik für die Analyse multideminsionaler Daten. Diese
Technik ermöglicht, Daten aus verschiedenen Sichtweisen darzustellen. \acrshort{OLAP} greift dabei auf die Daten im \acrshort{DWH} zu.
Ein anderes Verfahren ist Data Mining. Data Mining ist eine explorative Methode, die verschiedene statitische Verfahren auf einen Datenbestand anwendet,
um Muster und Zusammenhänge zu erkennen. Data Mining wird auch als Teilphase des Prozesses der Wissensentdeckung beschrieben 
\citeauthor[vgl.][142 f.]{linden_geschaftsmodellbasierte_2016}.
Neuere Tendenzen in \textit{\acrshort{BI}-Systeme}   führen weg von der strikten Trennung OLTP und OLAP und zur Auflösung des DWHs hinzu
Data Lake -> Speicherung der Rohdaten, bis sie benötigt werden -> führt dazu, dass der ETL-Prozess obsolet wird -> flexibler, kosteneffizienter
Trennung zwischen OLTP/OLAP-Systemen aufgehoben. Auf Kosten von Persistenz und dauerhafter Sicherung der Daten.
In-Memory Computing
Hadoop / MapReduce-Verfahren

Die Datenpräsention umfasst die Datendarstellung. Diese stellt Möglichkeit dar, die zuvor angewendeten 
Analyseverfahren und Berichtslösungen strukturiert und übersichtlich aufzubereiten Darstellung hochaggregierte, 
verdichtete quantitativer und qualitativer Information, die für Anwender rasch erfassbar und interpretierbar sind. 
Die Daten sind auf die Bedarfe der Anwender ausgerichtet und werden auf unterschiedlichen Aggregationsebenen visualisiert.(154)
Dabei spielt die Informationsvisualisierung eine wichtige Rolle. Es gibt verschiedene Ziele von Dashboards. 
