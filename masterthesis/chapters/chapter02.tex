...
Die Schichten der \textit{\acrshort{BI}-Systeme} können in einem Referenzarchitekturmodell abgebildet werden. Dieses Modell kann alle technischen Komponenten umfassen, die von 
der eigentlichen konkreten Implementierung abstrahiert werden müssen. Die Referenzarchitektur kann auch die Datenflüsse zwischen den verteilten Systemeinheiten und Komponenten erfassen \cite[vgl.][126 ff.]{linden_geschaftsmodellbasierte_2016}.
Eine schematische Darstellung der Schichten zeigt die \autoref{fig:bis}.
...
%Ein \acrlong{DWH} zeichnen die vier Merkmal themenorientierung, integration, zeitraumbezug, nicht-volatil
Die Datenintegration kann aufgrundlage eines multidimensionalen Datenmodells erfolgen. Ein \textit{\acrshort{DWH}} ist üblicherweise nach einem Datenwürfel (data cube) modelliert.
%Dieser Datenwürfel kann n-dimensionen haben. Der Datenwürfel besteht mit Fakten und Dimensionen aus zwei Elementen. Fakten sind die im Mittelpunkt stehenden Kennzahlen. Kennzahlen
%sind numerische Größen wie Gewinne und Erlöse. Die Dimensionen beschreiben die mögliche Sicht auf die Kennzahlen. So ist in einem multidimensionalen Datenmodell wie einem Datenwürfel, die Betrachtung der Kennzahlen aus 
%verschiedenen Dimensionen möglich. Ebenso ist durch Hierarchien die Unterteilung der Auswertungsebenen realisierbar.
...
Bei der Umsetzung mit Datenbanktechnik besteht die Möglichkeit, das multidimensionale Modell in einer relationalen Datenbank umzusetzen.  
...
Ein \textit{\acrlong{DWH}} bietet eine nicht-volatile Speicherung der Daten an. Damit ist sichergestellt, dass auf diesen Daten längerfristige Analysen durchgeführt werden können \cite[vgl.][136]{linden_geschaftsmodellbasierte_2016}.
...
Mit der Bereitstellung multidimensionaler
Daten in einem Datenwürfel durch das \acrlong{DWH}, werden \textit{\acrshort{OLAP}}-Funktionen wie drill-down, roll-up, slice oder dice ermöglicht. Drill-Down erlaubt so zum Beispiel einen 
...
Tabellen, Listen und Diagramme verwendet. Der Bericht kann bereitgestellt werden in Formaten wie PDF oder HTML. 


Im Gegensatz dazu ermöglichen Dashboards
einen interaktiven Zugang über eine Benutzungsoberfläche zu den relevanten Informationen.
...
Sie werden zur Kommunikation und Kollobartion auf oberster Managementeben genutzt. Die Grenzen zwischen den Dashboards bezüglich Reichweite und Zweck sind aber nicht trennscharf. So
...
Es wird dabei auf die aufwändigen \textit{\acrshort{ETL}-Prozesse} mitunter verzichtet \cite[Vgl.][86]{gronwald_integrierte_2020}, und die Daten werden zum Analysezeitpunkt bearbeitet.
