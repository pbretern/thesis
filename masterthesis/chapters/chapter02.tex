...
Populär wurde der Begriff der \textit{\acrshort{BI}} in den 1990er Jahren und fiel zusammen mit der 
...
die schnell befriedigt werden sollten \cite[vgl.][268 f.]{abts_grundkurs_2017}. Die Einrichtung dieser
Datenbanken zielt auf eine umfassende Informationsversorgung durch Verdichtung der Unternehmensdaten auf unterschiedlichen Stufen 
mit detaillierten Kennzahlen. Der Zugriff auf diese Informationen soll jederzeit erfolgen \cite[vgl.][267]{abts_grundkurs_2017}. 
...
Die wesentlichen Stufen eines \textit{\acrshort{BI}-Systems} umfassen die Bereiche der internen und externen Datenlieferanten, den Komplex der Datenintegration- und aufbereitung, 
...
Die Stufen der \textit{\acrshort{BI}-Systeme} können in einem Referenzarchitekturmodell abgebildet werden. Dieses Modell kann alle technischen Komponenten umfassen, die von 
der eigentlichen konkreten Implementierung abstrahiert werden müssen. Die Referenzarchitektur erfasst auch die Datenflüsse zwischen den verteilten Systemeinheiten und Komponenten\cite[vgl.][126 ff.]{linden_geschaftsmodellbasierte_2016}.
...
\begin{figure}[h]
    \centering
        ...
        \caption{Modelltypische Stufen eines Business-Intelligence-System}s
        \label{fig:bis}
\end{figure}
...
beseitigt. Des Weiteren werden die Daten verdichtet, d.h. es werden Summationen mit den Daten auf verschiedenen Ebenen durchgeführt und gespeichert. 
...
Schließlich werden die Daten in einem Ladeprozess in das \textit{\acrfull{DWH}} geladen \cite[vgl.][129 ff.]{linden_geschaftsmodellbasierte_2016}. 
...
die kleinere Datenspeichereinheiten darstellen und sich inhaltlich an späteren Abfrage- und Auswertungszwecken orientieren.} Aufgrundlage eines multidimensionalen
Datenmodells kann die Datenintegration erfolgen. Ein \acrshort{DWH} ist üblicherweise nach einem Datenwürfel modelliert. Die Daten entsprechen den Dimensionen des Würfels. 
Diese können über die Achsen des Würfels ausgewählt und kombiniert werden. Diese multidimensionale Datenstruktur bietet einen multidimensionalen Blick auf die Daten 
und erlaubt dadurch Vorberechnungen und einen schnellen Zugriff auf summierte Daten.


Ein \textit{\acrlong{DWH}} bietet eine dauerhafte Speicherung der Daten an. Damit ist sichergestellt, dass auf diesen Daten längerfristige Analysen durchgeführt werden können \cite[vgl.][136]{linden_geschaftsmodellbasierte_2016}.
...
Die letzte Stufe eines \textit{\acrshort{BI}-Systems} umfasst die Datenanalyse- und präsentation. Die Datenanalyse kann verschiedene Auswertungskonzepte aufweisen.
Im Rahmen herkömmlicher \textit{\acrshort{BI}-Systeme} wird \textit{\acrshort{OLAP}} angewendet. \textit{\acrshort{OLAP}} ist eine Anfragetechnik 
für die Analyse multidimensionaler oder relationaler Daten. Diese Technik erlaubt es, Daten aus verschiedenen Sichtweisen darzustellen. Mit der Bereitstellung multidimensionaler
Daten in einem Datenwürfel durch das \acrlong{DWH}, werden \textit{\acrshort{OLAP}}-Funktionen wie drill-down, roll-up, slice, dice\dots ermöglicht. Drill-Down erlaubt so zum Beispiel einen 
detaillierteren Blick auf die Daten, währenddessen mit der roll-up-Operation sich Daten auf einer höheren Hierarchiestufe betrachten lassen.

Ein anderes Verfahren ist Data Mining. Data Mining wird entweder synonym gesetzt mit dem Prozess der \acrfull{KDD} oder als Teilphase dieses Prozesses beschrieben
\cites[vgl.][6]{han_data_2012}[vgl.][142 f.]{linden_geschaftsmodellbasierte_2016}. Data Mining benutzt verschiedene statistische und mathematische Verfahren, um Muster und Trends in vor allem
großen Datenmengen zu entdecken. Klassische Data-Mining-Aufgaben sind Außreißer-Erkennung, Klassifikation, die Cluster- , Assoziations- und Regressionsanalysen\cite[vgl.][15 ff.]{han_data_2012}
...
Die Präsentation der Daten kann durch Standardberichte oder Dashboards erfolgen. Standardberichte werden durch BI-Tools generiert und den Benutzer:innen zur Verfügung gestellt. Diese 
statischen Berichte können nicht verändert werden. Nach \Citeauthor{few_information_2006} ist ein Dashboard ein \textquote{ ... a visual display of the most important 
information needed to achieve one or more objectives; consolidated and arranged on a single screen so the
information can be monitored at a glance.}\cite{few_information_2006} Im Rahmen von Unternehmen wird auch von Performance Dashboards
gesprochen. Performance Dashboards sind nach Eckerson "A performance dashboard is a 
layered information delivery system that parcels out information, insights, and 
alerts to users on demand so they can measure, monitor, and manage business performance more effectively."(S.10)
