...
\textit{\acrshort{BI}-Systeme} sind können in einer Referenzarchitekturmodell abgebildet werden. Dieses Modell umfasst alle technischen Komponenten, die von 
der konkreten Implementierung im Unternehmen aber abstrahiert werden.
...
Auf der ersten Ebene werden die Daten mithilfe eines \acrfull{ETL}-Prozesses bearbeitet. Diese Ebene wird auch als Staging Area bezeichnet.
...
Schließlich werden die Daten im Ladeprozess in ein \acrfull{DWH} geladen \cite[vgl.][129 ff.]{linden_geschaftsmodellbasierte_2016}. 

Die zweite Ebene entspricht dem \textit{\acrshort{DWH}} und ist nach \citeauthor{linden_geschaftsmodellbasierte_2016} der zentrale 
Bereich der \textit{\acrshort{BI}}-Referenzarchitekur. Ein \textit{\acrlong{DWH}} ist ein logisch zentralisiertes, unternehmensweites Datenhaltungssystem, das physisch 
...
Anwendung kommen. Im Rahmen der \textit{\acrshort{BI}-Systeme} wird häufig \acrfull{OLAP} angewendet. OLAP ist eine Anfragetechnik 
für die Analyse multideminsionaler Daten. Diese Technik ermöglicht, Daten aus verschiedenen Sichtweisen darzustellen. 
\textit{\acrshort{OLAP}} greift dabei auf die Daten im \acrshort{DWH} zu.
...
Neuere Tendenzen in \textit{\acrshort{BI}-Systeme} führen weg von der strikten Trennung zwischen OLTP und OLAP und zur Auflösung des \textit{\acrshort{DWH}} hinzu
