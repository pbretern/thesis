...
als Antwort auf die anhaltende Unsicherheit im Bereich der öffentlichen Gesundheit, hervorgerufen durch die Ausbreitung der Covid-19-Pandemie. 
Dieses visualisiert seitdem die gemeldeten Fälle weltweit. Das Dashboard wurde entwickelt, um Forschenden, Gesundheitsbehörden und der breiten Öffentlichkeit 
...
\textit{Crystal Reports} von \textit{SAP} oder \textit{Microsoft BI} von \textit{Microsoft}.
...
objektorientiert programmiert werden und dadurch leicht erweiterbar sowie eventuell von
...
aus bibliothekarischen Dienstleistungen aufzunehmen. Die Architektur des Dashboardes besteht aus drei Hauptteilen. Die Daten werden mit 
Import-Skripten aus den verschiedenen Datenquellen bezogen, mit einem ETL-Prozess bearbeitet und in ein Data Warehouse geladen. Das Data Warehouse 
...
Vom Entwicklungs-Team wurde sich gegen kommerzielle Lösungen entschieden und ein Dashboard, dass auf Ruby-on Rails basiert, entwickelt. Es zeigt verschiedene Daten wie Study-Room- oder Computer-Usage-Data.\footnote{{\url{https://library.ucmo.edu/stats/dashboard} Stand: 27.09.2020}}
...
In eine ähnliche Richtung wie das zu entwickelnde Tool, geht das an die an der TH Wildau entwickelte Software \textit{BibloVis}.
\textit{BibloVis} soll eine komfortable Alternative zu verstreut abgelegten Excel-Statistiken und zugangsgesteuerten online-Portalen bieten, mit welcher kurzfristig ein Gesamtüberblick erzeugt, 
längerfristig Analysen betrieben werden können, und somit die Entscheidungsfindung unterstützt wird. Das auf den Technologien HTML5 und Javascript basierende modular aufgebaute Server-Client Modell ermöglicht 
...
Grundlage sind Auswertungen von cvs- oder xml-Dateien. \cite{block_biblovis_2015}

Der Einsatz von Tableau an den \textit{Ohio State University Libraries} wird von \Citeauthor{murphy_data_2013} beschrieben. 
Anhand von zwei Projekten werden die Einsatzmöglichkeiten insbesondere der Datenvisualisierungsoptionen 
dieser Software beschrieben. Unter anderem wurde die transaction logs auf die 178 Bibliotheksleitfäden untersucht, 
...
der Literatur bei \Citeauthor{Spielberg}, der sich mit dem Thema der Bestandspflege an der
...
Das Kapitel beschreibt den Rahmen, in dem die prototypische Entwicklung eines Dashboardes eingebettet ist. Dabei wird herausgestellt, wie wichtig Statistik
