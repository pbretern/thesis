...
\section{Implementierung}
    ...
    einzelnen bibliothekarischen Daten. Innerhalb dieser Funktionen werden mit den Funktionen der Bibliothek Plotly Express \texttt{Plotly Graph Object Figure} geschaffen. 
    ...
    Die Funktionen der Dash-Komponenten binden die Funktionen der Datenvisualisierung so ein, dass die \texttt{Plotly Graph Object Figure} im Dashboard zur Anzeige gebracht werden können. 
    Weiterhin sind die Datenvisualisierungs-Funktionen zum Teil mit Decorator-Callback-Funktionen verknüpft, die es ermöglichen mit dem Dashboard zu interagieren. 
    So wurde in zwei Tab-Dateien die Logik von einfachen Callbacks implementiert. Weiterhin enthalten die Dateien jeweils eine Layoutfunktion, die das gesamte Layout des
    Tabs bündelt.
    ...
    instantiiert und die Methoden der Kindklassen aus diesem Modul auf diese angewendet. In der Regel geschieht dies ausserhalb der Datenvisualisierung-Funktionen.
    Für die Callback-Funktionalität werden aber die Objekte und die Methoden des Moduls \texttt{data\_prep} innerhalb der Datenvisualisierung-Funktionen aufgerufen.
    


    Schematisch zeigt die \autoref{fig:process tab} den Ablauf in den Tab-Dateien ohne die callback-Funktionen.
    Die Objekte werden in Plotly Graph Objects umgewandelt. Diese wiederum werden in Dash-Objekte umgewandelt und werden in einem
    Layout für das Tab zusammengefasst.
    ...
    Dabei sieht die grundsätzliche Logik der Funktionsaufrufe in den Tab-Dateien ohne die Callback-Funktionen wie folgt aus:
    ...
    Mit weiteren Funktionen von Plotly Express können \texttt{Plotly Graph Object Figure}  bearbeitet werden. 
    So kann durch die \texttt{update\_layout}-Funktion unter ansderem der Achsentitel, Höhe und Breite der Datenvisualisierung 
    ...
    Die \texttt{html\_fig}-Funktionen werden dann in einer Layoutfunktion eingebunden, die das Layout für den Tab bündelt und
    als Rückgabewert ebenso wie die \texttt{html\_fig}-Funktionen eine Div-Komponente der \texttt{dash\_html\_components} returniert.

    Die Callback-Funktionen wurden für zwei Dropdown-Menüs in zwei Tab-Dateien implementiert. 
    Der schematische Ablauf in der Tab-Datei mit den Callback-Funktionen zeigt die \autoref{fig:process tab_dash_cb}. 
    Der Callback ist als Dekorator für eine Funktion implementiert. 
    In der Implementierung wird ein Callback ausgelöst, wenn ein Wert über das Dropdown ausgewählt wird. 
    Der Callback übergibt diesen Wert an die Funktion \texttt{update\_output\_div()}. Die Funktion returniert unter anderem
    \texttt{fig\_data()}-Funktionen mit diesem Wert als Argument. 
    Diese Funktionen übergeben den Methoden der Kindklassen aus dem Modul dataprep das Argument. Diese erzeugen einen Dataframe basierend aus 
    den übergebenden Argument von dem Objekt und geben mit Hilfe der plotly-Funktionen eine \texttt{Plotly Graph Object Figure} der gefilterten Daten zurück.
    Der Callback übergibt dann die zurückgegebene Figur an die im Output angegebene Komponente.
    ...
    Das Dropdown-Menu wird aus uniquen Werte einer Dataframespalte erzeugt. Diese werden dann mit der Dropdoen-component von der dcc
    zu einem Dropdown-Element umgewandelt und mit einer id versehen. Die Werte der dropdown-Liste bestimmen als Input-Value in der 
    Calllback-Funktion den Output, indem sie einer fig\_data() als Argument übergeben werden.  
