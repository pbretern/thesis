...
\section{Implementierung}
    ...
    Innerhalb dieser Funktionen werden mit den Plotly-Funktionen der Bibliothek Plotly Express \texttt{Plotly Graph Object Figures} geschaffen. 
    ...
    zu Grunde. Es werden aus den berechneten Ergebnissen der Objekte direkt Dash-Komponenten mit Hilfe \texttt{dash\_bootstrap\_components} erstellt.}
    ...
    In der Plotly-Funktion, die das \texttt{Plotly Graph Object Figure} erzeugt, werden die durch die Kindklassen bearbeiteten Objekte (Dataframes, Series) 
    ...
    Es werden zusätzlich in dem Funktionsaufruf noch die x-Achse und y-Achse festgelegt. 
    ...
    das Aussehen des Div-Objektes definiert. Dabei wird unter anderem auf die externe css-Datei \texttt{layout.css} verwiesen, die in dem Verzeichnis
    \texttt{assets} des Dashboard-Verzeichnisses liegt.
    ...
    Die \texttt{html\_fig()}-Funktionen werden in einer Layoutfunktion eingebunden, die alle Funktionen dieser Art
    ...
    Folgendermaßen wird das im Programmcode für ein Diagramm der Lesesaalnutzung umgesetzt.
    ...
    In der folgenden Abbildung ist nocheinmal die Ablauflogik in den Tab-Dateien mit der Callback-Funktion zu sehen.
    ...
    Decorator-Callback-Funktion \texttt{@app.callback()} steuert die Auswahl der Tabs und ruft die jeweiligen Tabs beziehungsweise deren 
...
\section{Funktionsweise}
    ...
    \begin{figure}[H]
        \centering
            \includegraphics[width=15cm, height=2cm]{tabs}
            \caption{Tabs-Navigation}
            \label{fig:flask}
    \end{figure}

    Nach dem die Adresse Browser geöffnet wurde landet man auf dem ersten Tab.
