...
\section{Implementierung}
    ...
    An interaktiven Basisfunktionalitäten bietet Plotly Express Hover-Informationen der Datenpunkte, Zoom-In und Zoom-Out-Möglichkeiten,
    ...
    den Datenfluss an, während der rote Pfeil die Richtung der Methodenaufrufe / Initialisierung der Objelte angibt.}
    ...
    jeweils einen lokalen Ordner vorsieht. So zum Beisiel gibt es jeweils einzelne Ordner für bibliotheksspezifischen Daten wie die Budget- und Umsatzdaten.
    ...
    (Budget, Umsatz, Neuerwerbungslisten und Ausleihe (Anwendungsfall 2, 4-6)). Dennoch werden mit mit ihnen auch die bibliotheksinternen Daten
    ...
    zu Datenvisualisierungen nur noch weiterverarbeitet werden müssen. 
    ...
    nach einem Wert einer anderen Spalte ein und desselben pandas Dataframes. Mit dieser Methode soll so zum Beispiel konkret der Gesamtumsatz des 
    ...
    Die einzelnen Tabs wurden inhaltlich um bibliothekarische Tätigkeiten gruppiert. So werden in der \texttt{expenditures\_tab.py}
    ...
    Schematisch zeigt die \autoref{fig:process tab} die Ablauflogik in den Tab-Dateien ohne die callback-Funktionen anhand der 
    ...
    und auf ihn dann die Methode \texttt{total\_expnd\_net\_years()} ausgeführt und der Rückgabewert, der ein Dataframe ist, einer
    Variable zugewiesen.
    ...
    Im Programmcode wird das folgendermaßen für ein Diagramm der Lesesaalnutzung in der \texttt{loan\_read\_tab.py} umgesetzt.
    ...
    In der folgenden Abbildung ist die Ablauflogik in den Tab-Dateien mit der Callback-Funktion skizziert.
...
\section{Demonstration der Funktionalität}
    ...
    Im Folgenden wird die Funktionsweise des Systems dargelegt. Dabei wird zunächst auf die technischen Voraussetzungen eingegangen und dann
    ...
    Die Layout-Struktur besteht aus Divs, Rows, Header und Columns, und den Steuerelementen wie Dropdown-Menüs sowie den Darstellungselementen wie Diagrammen, Tabellen
    ...
    als default-value im Dashboard-Layout der \texttt{index.py} eingestellt. Im folgenden werden die Tabs und die in ihnen enthaltenen Informationen
    tabellarisch dargestellt, wobei die tabellarische Darstellung sich an den Rows der Layout-Struktur ausrichtet. 
    ...
    \begin{figure}[H]
        \centering
            \includegraphics[width=16cm, height=16.3 cm]{tab1}
            \caption{Tab1}
            \label{fig:tab1}
    \end{figure}
        ...
        \begin{tabular}{p{0.1\textwidth}p{0.5\textwidth}p{0.4\textwidth}p{0.25\textwidth}p{0.4\textwidth}p{0.5\textwidth}}
           ...
           \midrule
            1           &Gesamtumsatz Lieferanten nach Jahren &Die Einzelwerte der Lieferanten pro Jahr werden dargestellt.   &Umsatzdaten    &gestapeltes Balkendiagramm (horizontal)    &Plotly-Interaktivität (Aus- und Einblenden von Balken, Hover-Informationen)\\
                        &Top 10 Lieferanten mit Sonstige &Die 9 Lieferanten, bei denen der Umsatz am stärksten ist, werden dargestellt. Die restlichen werden in der Kategorie Sonstiges gruppiert.   &Umsatzdaten    &Balkendiagramm    &Plotly-Interaktivität (Aus- und Einblenden von Balken, Hover-Informationen)\\
            ...
            4           &Gesamtbudget Kostenstellen nach Jahren&Die Einzelwerte der Kostenstellen pro Jahr werden dargestellt.&Budgetdaten    &gestapeltes Balkendiagramm (horizontal)    &Plotly-Interaktivität (Aus- und Einblenden von Balken, Hover-Informationen)\\
                        &Top 5 Kostenstellen mit Sonstige&Die 4 Kostenstellen, bei den die Budgetkosten am größten sind, werden dargestellt. Die restlichen werden in der Kategorie Sonstiges gruppiert. &Budgetdaten    &Kreisdiagramm    &Plotly-Interaktivität (Aus- und Einblenden von Anteilen, Hover-Informationen)\\
...
%Den Tab \textit{Lesesaal und Ausleihe} zeigt \autoref{fig:tab2}.
    \begin{figure}[H]
        \centering
            \includegraphics[width=16cm, height=17.5cm]{tab2}
            \caption{Tab2}
            \label{fig:tab2}
    \end{figure}
...
%Folgende Diagramme, Tabellen und Steuerelemente werden im Tab \textit{Lesesaal und Ausleihe} dargestellt.
    ...
    \begin{table}[h]
        ...
        \begin{tabular}{p{0.1\textwidth}p{0.5\textwidth}p{0.4\textwidth}p{0.3\textwidth}p{0.4\textwidth}p{0.4\textwidth}}
           ...
           \midrule
            2           &Monatliche Anzahl der Nutzer:innen nach Service-Zeiten&Es wird der monatliche Verlauf nach den vier Service-Zeiten dargestellt.&Lesesaaldaten&Liniendiagramm&Auswahl des Zeitraums (Jahr) über Dropdown-Menü. Plotly-Interaktivität (Aus- und Einblenden von Linien, Hover-Informationen)\\
    ...
    \begin{figure}[H]
        \centering
            \includegraphics[width=16cm, height=17.5cm]{tab3}
            \caption{Tab3}
            \label{fig:tab3}
    \end{figure}
        ...
        \begin{tabular}{p{0.1\textwidth}p{0.5\textwidth}p{0.4\textwidth}p{0.3\textwidth}p{0.4\textwidth}p{0.4\textwidth}}
           ...
           \midrule
            ...
            2           &Bestandswachstum pro Jahr und Gesamt&Darstellung des absoluten und des relativen Wachstums nach Jahren.&Bestandsdaten&überlagertes Balkendiagramm&Plotly-Interaktivität (Aus- und Einblenden von Balken, Hover-Informationen)\\
                        &Top 10 RVK-Fachsys-tematiken pro Jahr&Die jährlichen Top 10 \textit{\acrshort{RVK}}-Fachsys-tematiken im Bestand werden dargestellt.&Bestandsdaten    &gestapeltes Balkendiagramm&Plotly-Interaktivität (Aus- und Einblenden von Balken, Hover-Informationen)\\
            \midrule
            3           &Top 10 RVK-Fachsys-tematiken Ausleihen&Es werden die Top 10 \textit{\acrshort{RVK}}-Fachsys-tematiken der Ausleihe vom kleinsten Wert zum größten des Bestandes (ohne Buchservice) dargestellt.&Ausleihdaten&Balkendiagramm&Plotly-Interaktivität (Aus- und Einblenden von Balken, Hover-Informationen)\\
                        &Top 10 RVK-Fachsys-tematiken Bestand&Es werden die Top 10 \textit{\acrshort{RVK}}-Fachsys-tematiken im Bestand vom größten Wert zum kleinsten dargestellt.&Bestandsdaten&Balkendiagramm&Plotly-Interaktivität (Aus- und Einblenden von Balken, Hover-Informationen)\\
...
dass diese Anforderungen für die vorliegenden Daten vom System erfüllt wurden.
...
Weiterhin können bestimmte Modifikationen im Teilsystem 1 eingestellt werden wie die Entfernung
bestimmter Zeichen. Diese Modifikationen sind aber sehr einfach und wurden aus der Voranalyse
...
Das betrifft zum Beispiel die Vielzahl an Lieferanten\footnote{in den Originaldaten} in dem Diagramm \enquote{Gesamtumsatz Lieferanten nach Jahren} im Tab \textit{Umsatz und Budget}. 
...
die vermutlich erst in einfachere Strukturen aufgelöst werden hätte müssen.\\
...
\begingroup
    ...
    \begin{table}[h]
        ...
        \begin{tabular}{p{0.4\textwidth}p{0.5\textwidth}p{0.7\textwidth}}
           ...
           Das System zeigt die ausleihstärksten Titel absteigend nach Anzahl und Jahr an.&Tabellarische Darstellung der 5 besonders nachgefragten Titel nach Jahren&Die Anzahl der anzuzeigenden Titel kann im Programmcode der tabdatei beim Methodenaufruf als Parameter eingestellt werden.\\
           Das System zeigt die Verteilung der ausgeliehenen Titel in der \textit{\acrshort{RVK}}-Fachsys-tematiken pro Jahr an.&Ausleihe Top 10 RVK-Fachsystematiken mit Buchservice und Sonstige&Die Anzahl der \textit{\acrshort{RVK}}-Fachsystematiken kann im Programmcode der tabdatei beim Methodenaufruf als Parameter eingestellt werden.\\
           Das System zeigt die Top-\textit{\acrshort{RVK}}-Fachsys-tematiken der Ausleihe an.&Ausleihe Top 10 RVK-Fach-systematiken pro Jahr\footnotemark&Die Anzahl der \textit{\acrshort{RVK}}-Fachsystematiken kann im Programmcode beim Methodenaufruf als Parameter eingestellt werden.\\
...
\begingroup
    ...
    \begin{table}[h]
        ...
        \begin{tabular}{p{0.4\textwidth}p{0.5\textwidth}p{0.7\textwidth}}
           ...
           \midrule
           Das System filtert die betreffenden Datensätze nach Jahr. Das System zeigt diese Datensätze mit Datenvisualisierungen an.&-&Durch die Teilsysteme 2  und 3 gewährleistet.\\
           Das System zeigt die Nutzung des Lesesaals nach Monat und Jahr, gruppiert in vier Service-Zeiten an.&Monatliche Anzahl der Nutzer:innen nach Service-Zeiten& Die Jahre können im Dropdown-Menü im Frontend ausgewählt werden.\\
...
\begingroup
    ...
    \begin{table}[h]
        ...
        \begin{tabular}{p{0.4\textwidth}p{0.5\textwidth}p{0.7\textwidth}}
           ...
           Das System zeigt die Anzahl der Titel nach Medienart an.&-&Diese Anforderung wurde zwar im Programmcode implementiert, aber nicht im Dashboard umgesetzt, da das gedruckte Buch als Medium dominiert.\\
...
\begingroup
    ...
    \begin{table}[h]
        ...
        \begin{tabular}{p{0.4\textwidth}p{0.5\textwidth}p{0.7\textwidth}}
           ...
           \midrule
           Das System filtert die betreffenden Datensätze. Das System zeigt diese Datensätze mit Datenvisualisierungen an.&-&Durch die Teilsysteme 2 und 3 gewährleistet.\\
           Das System zeigt die Top-\textit{\acrshort{RVK}}-Fachsyste-matiken des Bestandes pro Jahr an.&Top 10 RVK-Fachsyste-matiken pro Jahr&Die Anzahl der Fachsystematiken kann im Programmcode beim Methodenaufruf als Parameter eingestellt werden.\\
           Das System zeigt die Gesamtzahl der Titel nach Jahren an.&Bestandswachstum pro Jahr und Gesamt&-\\
           Das System zeigt die Anzahl der Titel nach Medienart an.&-&Diese Anforderung wurde zwar im Teilsystem 2 implementiert, aber wird nicht als Diagramm im Dash-board gezeigt, da zu wenig Medienarten in der Bibliothek vertreten sind und das gedruckte Buch überwiegt.\\
           Das System zeigt die Top-\textit{\acrshort{RVK}}-Fachsyste-matiken des Bestandes insgesamt an.&Top 10 RVK-Fachsyste-matiken Bestand&Die Anzahl der Fachsystematiken kann im Programmcode beim Methodenaufruf als Parameter eingestellt werden.\\
        \bottomrule
        \end{tabular}
        \end{adjustbox}
        \caption{%
...
\begingroup
    ...
    \begin{table}[h]
        ...
        \begin{tabular}{p{0.4\textwidth}p{0.5\textwidth}p{0.7\textwidth}}
           ...
           Das System zeigt die umsatzstärksen Lieferanten im Gesamtzeitraum an.&Top 10 Lieferanten mit Sonstige  &Die Anzahl der Lieferanten kann im Programmcode beim Methodenaufruf als Parameter eingestellt werden.\\
           ...
           Das System zeigt die kostenintensivsten Kostenstellen im Gesamtzeitraum an.&Top 5 Kostenstellen mit Sonstige&Die Anzahl der Kostenstellen kann im Programmcode beim Methodenaufruf als Parameter eingestellt werden.\\
        \bottomrule
        \end{tabular}
        \end{adjustbox}
        \caption{%
