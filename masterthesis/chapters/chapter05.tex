...
ausewertet. Als Ergebnis steht ein Dashboard, dass die bibliothekarischen Datensets aus den drei Bereichen Umsatz und Budget,
Lesesaal und Ausleihe, und der Bestandsentwicklung darstellt. Damit ist Umsetzung des Proof-of-Concepts gelungen.
...
durchgeführt werden können. Bündelung der Daten an einem zentralen Platz. Ebenso ist es gelungen eine grafische Oberfläche für die Diagramme anzubieten. 
Diese Diagramme sind zum einen mit Python Bibliothek Plotly interaktiv. Es konnte auch ein bisschen Interaktivität hinzu programmiert werden.
...
den anderen beiden heraus. Das hatte auch zur Folge, dass es die anderen beiden Teilbereiche beinflusst hat.

Die Programmiersprache Python und insbesondere die Bibliothek pandas haben sehr zum Gelingen dieses Projektes beigetragen. Python mit
der einfachen Syntax und Flexibilität und pandas als mächtige Bibliothek für die Datenanalyse und Datenmanipulation waren sehr hilfreich.
Ebenso ist das Zusammenspiel zwischen Dash und Plotly für grafische Darstellung sehr glücklich, da die Programmiersprache Python nicht verlassen
wurde und die JavaScript-Frameworks weg abstrahiert wurden.

Leider war in der Zeit der Bearbeitung keine Zeit mehr für die Entwicklung eines Standardberichtes.
...
geliefert. Dadurch entsteht eine Schieflage im Vergleich zwischen Bestandsdaten und Ausleihdaten in Bezug auf die Größe des Bestandes.
Es müssten für einen Ablauf die bibliotheksinternen Prozesse angepasst werden und schließlich der Ablauf im vorliegenden Projekt.


Data is messy - Library data too!
Weitere Hürden liegen in den Daten selbst, die überwunden werden mussten. Zu nennen wären hier fehlende Daten in den Umsatz- und Budgetdaten.
Diese fehlenden Daten können Fehler in der Darstellung im Dashboard erzeugen.\footnote{Ebenso fehlen relevante Lieferanten, die nicht im LBS auftauchen.}
Bei dem monatlichen Abzug der Neuerwerbungsdaten aus dem CBS werden unter Umständen Duplikate mit exportiert. Diese Duplikate entstehen durch
die Mehrfachexemplare, die an demselben Datensätzen hängen. Diese lassen sich zwar relativ einfach als Duplikate über die Signatur erkennen.
Darunter fallen dann aber auch alle ebooks, da sie diesselbe Signatur besitzen.

Mit all diesen Problemen muss ein Umgang gefunden werden, aber deswegen ist es so schwer etwas Allgemeingültiges zu programmieren, dass die Fehler
ausschließt. Deswegen bedarf es einer gründlichen Datenanalyse im vorhinein.

Die Grenzen der Darstellbarkeit der Daten Diagramme sind erreicht, wenn zu viele diskrete Werte dargestellt werden müssen.
Das betrifft zum Beispiel die Vielzahl an Lieferanten in dem Diagramm ... Dort kann anhand der Farbe in den gestapelten Balkendiagramm
nicht mehr unterschieden werden, da die Color-Palette nur eine bestimmte Anzahl an diskreten Werten darstellen kann.\footnote{https://plotly.com/python/discrete-color/}
Ebenso betrifft dies, die Auswertung der Daten nach den RVK-Systematiken beziehungsweise nach deren Untergruppen. Erste Analysen der Daten haben
festgestellt, dass es eine nicht darzustellbare Anzahl an Fachsystematiken und im Bestand der Bibliothek gibt, die nicht darstellbar ist, ohne
Verletzung der im \autoref{chap:two} dargelegten Konventionen zu den Datenvisualisierungen.

Das sind Schwierigkeiten, die in alle Teilsysteme mehr oder weniger eingreifen und die Programmierung entscheidend mitbestimmen.
...
da die Zeit nicht mehr ausgereicht hat. Einerseits aufgrund einer komplexeren Tabellenstruktur (2) und andererseits wegen
der begrenzten Zeitraums nicht möglich (2, 7).\\
...
\begingroup
    ...
    \begin{table}[h]
        ...
        \begin{tabular}{p{0.4\textwidth}p{0.5\textwidth}p{0.5\textwidth}}
           ...
           Anforderung des Anwendungsfalls        &Titel des Diagramms / Tabelle im Dashboard &Bemerkung\\
           \midrule
           Die Bibliotheksleitung und die Bibliotheksmitarbeiter:innen lassen sich auf der \textit{\acrshort{GUI}} das Bestandswachstum insgesamt und nach \textit{\acrshort{RVK}}-Fachsystematiken anzeigen.&-&Durch die Teilsysteme 2 und 3 gewährleistet.\\
...
\begingroup
    ...
    \begin{table}[h]
        ...
        \begin{tabular}{p{0.4\textwidth}p{0.5\textwidth}p{0.5\textwidth}}
           ...
           \midrule
           Die Bibliotheksleitung und die Bibliotheksmitarbeiter:innen lassen sich auf der \textit{\acrshort{GUI}} das Bestandswachstum insgesamt und nach \textit{\acrshort{RVK}}-Fachsystematiken anzeigen.&-&Durch die Teilsysteme 2 und 3 gewährleistet.\\
           Die Bibliotheklsleitung und die Bibliotheksmitarbeiter:innen können sich den Lieferanten auswählen und den Gesamtumsatz und den Umsatz pro Jahr ansehen.&Top 10 Systematikgruppen pro Jahr&Die Anzahl der Fachsystematiken kann im Backend als Parameter eingestellt werden.\\
           Das System zeigt darüberhinaus die Gesamtzahl der Titel nach Jahren an.&Bestandswachstum pro Jahr und Gesamt&-\\
           Das System zeigt die Top\textit{\acrshort{RVK}}-Fachsystematiken des Bestandes insgesamt an.&Top 10 Systematikgruppen Bestand&Die Anzahl der Fachsystematiken kann im Backend als Parameter eingestellt werden.\\
        \bottomrule
        \end{tabular}
