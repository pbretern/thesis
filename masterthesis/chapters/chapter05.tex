...
\section{Implementierung}
    ...
    \texttt{Dash\_bootstrap\_components} ist eine weitere Bibliothek, die zum Einsatz in diesem Projekt kommt. Sie unterstützt die graphische Umsetzung
    ...
    Das Teilsystem 1 unterstützt bereits den Import der Daten in Formaten wie txt, tsv, xlsx und xls. Diese Formate werden im Programmcode des Teilsystems 1 festgelegt
    ...
    \autoref{fig:umsatzuebersicht_txt} zeigt eine Umsatz-Datei, die im txt-Format gespeichert wurde.
    ...
    und auf ihn die Methode \texttt{total\_expnd\_net\_years()} ausgeführt und der Rückgabewert der Methode, der einem Dataframe entspricht, einer
    Variable zugewiesen.
...
\section{Demonstration der Funktionalität}
    ...
    gestartet wird. Das Dashboard ist die graphische Umsetzung des Teilsystem 2 und Teilsystem 3. Beim Dashboard wird kurz auf das Layout
    eingegangen, bevor die Funktionsweise und die Datenvisualisierungen besprochen werden.


    \subsection{Technische Voraussetzungen}
    Der Programmcode zum Projekt ist auf GitHub zu finden.\footnote{ \url{https://github.com/pbretern/library-dashboard-system} Version 1.0} Dort gibt es weitere Informationen zur Installation.
    ...
    Mit dem Programmcode werden keine bibliothekarischen Daten mitgeliefert.\footnote{ Testdaten werden nach der Veröffentlichung des Repositoriums
    sukzessive zur Verfügung gestellt. Im Moment stehen nur pseudonymisierte und randomisierte Umsatz- und Budgetdaten mit der Version 1.0 zur Verfügung.}
    ...
    \begin{figure}[H]
        \centering
            ...
            \caption{Tab1 - Umsatz und Budget}
            \label{fig:tab1}
    \end{figure}
    %\end{sidewaysfigure}
...
\KOMAoptions{paper=A3,paper=portrait}
\recalctypearea
\textit{Tab2 - Lesesaal und Ausleihe}\footnote{ Die Skalen und Legenden der Diagramme sowie die Tabelle wurden unscharf gemacht.
...
%Den Tab \textit{Lesesaal und Ausleihe} zeigt \autoref{fig:tab2}.
    \begin{figure}[H]
        \centering
            ...
            \caption{Tab2 - Lesesaal und Ausleihe}
            \label{fig:tab2}
    \end{figure}
...
\clearpage
    ...
    \begin{figure}[H]
        \centering
            ...
            \caption{Tab3 - Neuerwerbungen und Bestand}
            \label{fig:tab3}
    \end{figure}
...
Mehrfachexemplare, die an nur einem Datensatz hängen. Diese lassen sich zwar als Duplikate relativ einfach über die Signatur erkennen und entfernen.
Darunter würden dann aber auch beispielsweise alle ebooks fallen, da sie die gleiche Signatur aufweisen. Mit diesen Ausnahmen innerhalb der Daten musste und wurde ein Umgang gefunden.
...
die vermutlich erst in einfachere Strukturen aufgelöst hätte werden müssen. Aber zum Teil wird der Anwendungsfall 2
mit im Anwendungsfall 1 bearbeitet, da ein Teil der Buchservice-Daten aus dem \textit{\acrshort{CBS}} mit in den Ausleihdaten
